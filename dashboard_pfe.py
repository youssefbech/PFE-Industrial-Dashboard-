...
def init_mqtt():
    q = queue.Queue()
    # Utilisation de la version 2 du client pour éviter les warnings
    client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, userdata=q)
    
    # Configuration de l'authentification Flespi
    client.username_pw_set(FLESPI_TOKEN, "") 
    
    client.on_message = on_message
    
    # Connexion au broker distant au lieu de localhost
    client.connect("mqtt.flespi.io", 1883, 60)
    
    # Assure-toi que la Raspberry Pi envoie sur ce type de topic : motor/M1/data
